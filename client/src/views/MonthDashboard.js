...
* Description
* -----------------------------------------------------------------------------------
* Component for monthly dashboard view
*
* -----------------------------------------------------------------------------------
*
* Revision History
...
function MonthDashboard() {
  ...
  const [modalType, setModalType] = useState(""); // "mandays", "cost", "bugs", "defectdensity", "kloc", or "projects"
  ...
  if (!year || !month) {
    return (
      <div className="content">
        <h2>Month Dashboard</h2>
        <p>Please select a month from the sidebar.</p>
      </div>
    );
  }

  // Collect all projects and releases for this month
  ...
  // Combine all rows from all projects and releases
  let allRows = [];
  let projectsList = [];
  let numberOfProjects = 0;
  let projectWiseData = []; // For modal display
  
  projectsList = Object.keys(monthData);
  numberOfProjects = projectsList.length;
  
  // Calculate project-wise aggregation and combine all rows
  const costPerManDay = 250;
  projectsList.forEach(project => {
    const projectData = monthData[project];
    let projectRows = [];
    let numberOfReleases = 0;
    
    if (Array.isArray(projectData)) {
      // Direct array (no releases)
      projectRows = projectData;
      numberOfReleases = 0;
    } else {
      // Object with releases
      const releasesList = Object.keys(projectData);
      numberOfReleases = releasesList.length;
      projectRows = releasesList.flatMap(release => {
        const releaseData = projectData[release];
        // Handle both { rows: [...], categoryData: {...} } and direct array format
    ...
    const manDays = projectRows.find(r => r.row === "Man Days")?.deliverables || 0;
    const cost = manDays * costPerManDay;
    
    // Calculate bugs and deliverables for defect density
    const projectBugs = projectRows.reduce((sum, r) => sum + (Number(r.bugs) || 0), 0);
    const projectDeliverables = projectRows.reduce((sum, r) => sum + (Number(r.deliverables) || 0), 0);
    
    // Extract KLoc data and IT bugs for bug density calculation
    const projectKloc = projectRows.find(r => r.row === "CD (Kloc)")?.deliverables || 0;
    const projectItBugs = projectRows.find(r => r.row === "IT(No of Test cases)")?.bugs || 0;
    const projectDefectDensity = projectKloc > 0 
      ? (projectItBugs / projectKloc).toFixed(2)
      : "0.00";
    ...
    projectWiseData.push({
      ...
      cost,
      bugs: projectBugs,
      defectDensity: projectDefectDensity,
      kloc: projectKloc
    });
  });
  ...
  // Convert to array and filter out rows without both deliverables and bugs
  const aggregatedRows = Object.values(aggregatedByRow).filter(r => 
    r.deliverables !== undefined && r.bugs !== undefined
  );
  ...
  // Get Testing Phase Bugs (Integration Testing, UAT, Go Live)
  ...
  // Calculate totals
  const totalKloc = aggregatedRows.find(r => r.row === "CD (Kloc)")?.deliverables || 0;
  ...
  const productivity = totalManDays > 0
    ? (totalDeliverables / totalManDays).toFixed(2)
    : "-";

  // Extract and aggregate category data from all projects
  let categoryData = {};
  ...
  projectsList.forEach(project => {
    ...
    if (Array.isArray(projectData)) {
      ...
      if (projectData.categoryData) {
        Object.keys(projectData.categoryData).forEach(category => {
          if (!categoryData[category]) {
            categoryData[category] = 0;
          }
          categoryData[category] += projectData.categoryData[category];
        });
      }
    } else {
  ...
  // Testing Phase Bugs Comparison Chart Data
  ...
  // Pie Chart 3: Bug Testing Phase
  const testingPhasePieData = {
  ...
  return (
    <div className="content">
      <div className="d-flex justify-content-between align-items-center mb-1">
        <h2 className="mb-0" style={{ fontSize: "1.5rem" }}>{month} {year}</h2>
      ...
      <p style={{ fontSize: "0.85rem", marginBottom: "0.5rem" }}>Combined data for all projects in this month.</p>

      {/* Summary Cards */}
      <Row>
        <Col xl="1" lg="3" md="4" sm="6" style={{ flex: "1 1 14.28%", maxWidth: "14.28%" }}>
          <Card className="card-stats" onClick={openProjectsModal} style={{ cursor: "pointer" }}>
            <CardBody style={{ padding: "10px" }}>
              <Row>
                <Col xs="12">
                  <div className="info-icon text-center icon-primary" style={{ marginBottom: "8px" }}>
                    <i className="tim-icons icon-notes" style={{ fontSize: "1.5rem" }} />
                  </div>
                  <div className="numbers text-center">
                    <p className="card-category" style={{ fontSize: "0.65rem", marginBottom: "3px" }}>Projects</p>
                    <CardTitle tag="h3" style={{ fontSize: "1.5rem", marginBottom: "0" }}>{numberOfProjects}</CardTitle>
                  </div>
                </Col>
              </Row>
            </CardBody>
              ...
              <Row>
                <Col xs="12">
                  <div className="info-icon text-center icon-warning" style={{ marginBottom: "8px" }}>
                    <i className="tim-icons icon-trophy" style={{ fontSize: "1.5rem" }} />
                  </div>
                  <div className="numbers text-center">
              ...
              <Row>
                <Col xs="12">
                  <div className="info-icon text-center icon-danger" style={{ marginBottom: "8px" }}>
                    <i className="tim-icons icon-alert-circle-exc" style={{ fontSize: "1.5rem" }} />
                  </div>
                  <div className="numbers text-center">
                    <p className="card-category" style={{ fontSize: "0.75rem", marginBottom: "3px" }}>Bugs</p>
              ...
              <Row>
                <Col xs="12">
                  <div className="info-icon text-center icon-success" style={{ marginBottom: "8px" }}>
                    <i className="tim-icons icon-chart-bar-32" style={{ fontSize: "1.5rem" }} />
                  </div>
                  <div className="numbers text-center">
                    <p className="card-category" style={{ fontSize: "0.75rem", marginBottom: "3px" }}>Defect Density</p>
              ...
              <Row>
                <Col xs="12">
                  <div className="info-icon text-center icon-info" style={{ marginBottom: "8px" }}>
                    <i className="tim-icons icon-time-alarm" style={{ fontSize: "1.5rem" }} />
                  </div>
                  <div className="numbers text-center">
                    <p className="card-category" style={{ fontSize: "0.75rem", marginBottom: "3px" }}>Productivity</p>
                    <CardTitle tag="h3" style={{ fontSize: "1.5rem", marginBottom: "0" }}>{productivity}</CardTitle>
                  </div>
                </Col>
              </Row>
            </CardBody>
          ...
          <Card className="card-stats" onClick={openManDaysModal} style={{ cursor: "pointer" }}>
            <CardBody style={{ padding: "10px" }}>
              <Row>
                <Col xs="12">
                  <div className="info-icon text-center icon-primary" style={{ marginBottom: "8px" }}>
                    <i className="tim-icons icon-calendar-60" style={{ fontSize: "1.5rem" }} />
                  </div>
                  <div className="numbers text-center">
          ...
          <Card className="card-stats" onClick={openCostModal} style={{ cursor: "pointer" }}>
            <CardBody style={{ padding: "10px" }}>
              <Row>
                <Col xs="12">
                  <div className="info-icon text-center icon-success" style={{ marginBottom: "8px" }}>
                    <i className="tim-icons icon-money-coins" style={{ fontSize: "1.5rem" }} />
                  </div>
                  <div className="numbers text-center">
      ...
      <Row className="mb-2">
        {/* Left Column: Project Totals Table and Bar Chart */}
        <Col md="6">
          {/* Deliverables vs Bugs Horizontal Bar Chart */}
          <Card className="card-chart" style={{ marginBottom: "1rem" }}>
            <CardHeader style={{ padding: "0.75rem 1rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h5 className="card-category" style={{ fontSize: "0.85rem", marginBottom: "0" }}>Phase-wise Analysis - Deliverables vs Bugs</h5>
            </CardHeader>
          ...
          <Card className="mb-2">
            <CardHeader className="d-flex justify-content-between align-items-center pe-3" style={{ padding: "0.75rem 1rem" }}>
              <CardTitle tag="h4" style={{ fontSize: "1rem", marginBottom: "0" }}>Month Totals</CardTitle>
              <div className="d-flex align-items-center gap-2">
              ...
              <Table className="tablesorter" style={{ fontSize: "0.75rem", marginBottom: "0", tableLayout: "fixed", width: "100%" }}>
                    <thead className="text-primary">
                      <tr style={{ lineHeight: "1.2" }}>
                        ...
                        {showAverages && <th className="text-center" style={{ padding: "0.4rem 0.3rem", width: "12.5%" }}>Avg D.</th>}
                        {showAverages && <th className="text-center" style={{ padding: "0.4rem 0.3rem", width: "12.5%" }}>Avg B.</th>}
        ...
        <div style={{ 
          ...
          <div style={{
            ...
            <h3 style={{
              ...
              {modalType === "projects" && "Projects Details"}
            </h3>
            <button 
              onClick={toggleModal}
              style={{
              ...
              <Table className="tablesorter" style={{ marginBottom: 0, width: "100%", tableLayout: "fixed" }}>
                <thead>
                  <tr style={{ 
                    ...
                    <th style={{ fontSize: "1.1rem", color: theme === themes.light ? "#1e1e2e" : "#ffffff", padding: "12px" }}>Project Name</th>
                    <th className="text-center" style={{ fontSize: "1.1rem", color: theme === themes.light ? "#1e1e2e" : "#ffffff", padding: "12px" }}>Releases</th>
                  </tr>
                </thead>
                <tbody>
                  {projectWiseData.map((item, idx) => (
                    <tr key={idx} style={{ 
                      background: theme === themes.light
                      ...
                      <td style={{ 
                        ...
                        <Link 
                          to={`/admin/${year}/${month}/${item.project}`}
                          style={{ 
                            color: "#1d8cf8", 
                            textDecoration: "none",
                      ...
                      <td className="text-center" style={{ padding: "10px 12px" }}>{item.numberOfReleases}</td>
                    </tr>
                  ))}
                  {projectWiseData.length > 0 && (
                    <tr style={{ 
                      ...
                      <td style={{ padding: "12px" }}>Total: {projectWiseData.length} Project{projectWiseData.length !== 1 ? 's' : ''}</td>
                      <td className="text-center" style={{ padding: "12px" }}>{projectWiseData.reduce((sum, p) => sum + p.numberOfReleases, 0)}</td>
                    </tr>
                  )}
                </tbody>
              </Table>
              ...
              <Table className="tablesorter" style={{ marginBottom: 0, width: "100%", tableLayout: "fixed" }}>
                <thead>
                  <tr style={{ 
                    ...
                    <th style={{ fontSize: "1.1rem", color: theme === themes.light ? "#1e1e2e" : "#ffffff", padding: "12px" }}>Project</th>
                ...
                <tbody>
                  {projectWiseData.map((item, idx) => (
                    <tr key={idx} style={{ 
                      ...
                      {modalType === "kloc" && <td className="text-center" style={{ padding: "10px 12px" }}>{item.kloc}</td>}
                    </tr>
                  ))}
                  {projectWiseData.length > 0 && (
                    <tr style={{ 
                      ...
                      {modalType === "kloc" && <td className="text-center" style={{ padding: "12px" }}>{totalKloc}</td>}
                    </tr>
                  )}
                </tbody>
              </Table>
